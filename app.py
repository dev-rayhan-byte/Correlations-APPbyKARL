...
def heatmap_matplotlib(corr: pd.DataFrame, decimals: int = 2):
    ...
    cell = 0.55 if n >= 12 else 0.7
    w = max(6, min(0.9 * n, 24))       # cap size to avoid giant images
    h = max(5, min(0.9 * n, 24))
    fig, ax = plt.subplots(figsize=(w, h))

    show_annot = n <= 18  # avoid overlapping numbers for large matrices
    ...
    if show_annot:
        # re-apply font size for annotations
        for t in ax.texts:
...
# ---------- Title ----------
st.title("Correlation & Visualization Tool powered by KARL (Pro Edition)")
...
# Export options (used by all tabs)
...
""", unsafe_allow_html=True)
    ...
    # Precompute numeric df & corr for reuse
    ...
    # ---------------------------
    with tab1:
        # Improved interactive heatmap (Plotly)
        fig_px = px.imshow(
            corr.values,
        ...
        fig_px.update_traces(xgap=1, ygap=1)  # gridlines for visual clarity
        ...
        # Download (Matplotlib render for publication quality)
        fig_dl = heatmap_matplotlib(corr, decimals=heatmap_decimals)
        # Convert to bytes in requested format & DPI
        bytes_dl = fig_to_bytes(export_fmt, export_dpi)
        st.download_button(
            f"Download Heatmap ({export_fmt.upper()}, {export_dpi} DPI)",
            bytes_dl,
            f"correlation_heatmap.{ 'jpg' if export_fmt=='jpg' else ('tiff' if export_fmt=='tiff' else 'png') }",
    ...
    with tab2:
        ...
        # Matplotlib export for chosen format/DPI
        ...
        st.download_button(
            ...
            f"scatter.{ 'jpg' if export_fmt=='jpg' else ('tiff' if export_fmt=='tiff' else 'png') }",
            mime={"png": "image/png", "jpg": "image/jpeg", "tiff": "image/tiff"}[export_fmt]
        )
        plt.close()
    ...
    with tab3:
        ...
        if len(selected_cols) > 1:
            ...
            fig_px_pair.update_layout(
                margin=dict(l=10, r=10, t=40, b=10),
                font=dict(family="DejaVu Serif" if paper_mode else None, size=14 if paper_mode else 12)
            )
            st.plotly_chart(fig_px_pair, use_container_width=True)

            # High-res Matplotlib/Seaborn pairplot for download
            ...
            st.download_button(
                ...
                fig_to_bytes(export_fmt, export_dpi),
                f"pairplot.{ 'jpg' if export_fmt=='jpg' else ('tiff' if export_fmt=='tiff' else 'png') }",
                mime={"png": "image/png", "jpg": "image/jpeg", "tiff": "image/tiff"}[export_fmt]
            )
            plt.close(g.fig)
        else:
    ...
    with tab4:
        ...
        corr_unstacked = corr_unstacked[corr_unstacked < 0.9999]  # remove self-corr
